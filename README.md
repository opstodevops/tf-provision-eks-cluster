...
Steps to install [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html)

After installing the AWS CLI. Configure it to use AWS credentials.
...
Initalize Terraform workspace, which will download the provider and initialize it with the values provided in the `terraform.tfvars` file.
...
Then, provision EKS cluster by running `terraform apply`. This will 
...
To configure kubetcl, install both [kubectl](https://kubernetes.io/docs/tasks/tools/install-kubectl/) and [AWS IAM Authenticator](https://docs.aws.amazon.com/eks/latest/userguide/install-aws-iam-authenticator.html).

The following command will get the access credentials for the EKS cluster and automatically
configure `kubectl`.
...
View these outputs again by running:
...
To verify that EKS cluster is configured correctly and running, 
install a Kubernetes dashboard and navigate to it in local browser. 
...
Creating a proxy server that will allow navigation to the dashboard 
from the browser on local machine. This will continue running until the process is stopped by pressing `CTRL + C`.

```shell
$ kubectl proxy
```

Access the Kubernetes dashboard [here](http://127.0.0.1:8001/api/v1/namespaces/kubernetes-dashboard/services/https:kubernetes-dashboard:/proxy/).
...
To use the Kubernetes dashboard, an authorization token is required. 
Authenticating using `kubeconfig` is **not** an option. Read more about
...
Select "Token" on the Dashboard UI then copy and paste the entire token 
received into the 
...
to sign in. Now sign in to the dashboard for EKS Kubernetes cluster.
